...
describe("TwapOracleLibrary", () => {
  beforeEach(async () => {
    ...
    uniswapV3Factory = (await (await UniswapV3FactoryFactory).deploy()) as UniswapV3Factory
    ...
    oracleLibraryTest = (await (await OracleLibraryTestContract).deploy()) as TwapOracleLibraryTest
    ...
    // deploy strategy factory
    shareHelper = (await (await ShareHelperTestFactory).deploy()) as TwapShareHelperTest
  ...
  describe("#getChainlinkPrice", async () => {
    it("should return correct chainlink price", async () => {
      ...
      expect(await oracleLibraryTest.getChainlinkPrice(chainlinkRegistry.address, token0A, token1A, 3600)).to.equal(
        "3000000000000000000000"
      )

      await chainlinkRegistry.setAnswer("400000000000", "100000000")
  ...
  describe("#getPriceInUSD", async () => {
    it("should return correct token0 price in USD", async () => {
      let useTwap: [boolean, boolean] = [true, false]
      const price = await oracleLibraryTest.getPriceInUSD(
        factory.address,
        pool.address,
        chainlinkRegistry.address,
        await pool.token0(),
    ...
    it("should return correct token0 & token1 price in USD - pool of ETH-USDT - decimals 18-6 - twap for token0", async () => {
      ...
      // get uniswap pool instance
      let ethUsdtpool = (await ethers.getContractAt(
        "UniswapV3Pool",
        await uniswapV3Factory.getPool(token1.address, token2.address, "3000")
      )) as UniswapV3Pool

      let chainlinkRegistryEthUsdt = (await (await ChainlinkRegistryMockFactory).deploy(
      ...
      if (poolToken0 == token1.address) {
        console.log("in if")
        // initialize the pool
        await ethUsdtpool.initialize(encodePriceSqrt(expandTo18Decimals(10), "30000000000"))
        ...
        await chainlinkRegistryEthUsdt.setAnswer("0", "100000000")
      } else {
        console.log("in else")

        // initialize the pool
        await ethUsdtpool.initialize(encodePriceSqrt("30000000000", expandTo18Decimals(10)))
        ...
        await chainlinkRegistryEthUsdt.setAnswer("0", "300000000000")
      }
      ...
      await periphery.swap(ethUsdtpool.address, false, "10000000000000000000", expandToString(sqrtPriceLimitX96))

      let useTwap: [boolean, boolean] = [true, false]
      const price0 = await oracleLibraryTest.getPriceInUSD(
        factory.address,
        ethUsdtpool.address,
      ...
      const price1 = await oracleLibraryTest.getPriceInUSD(
        factory.address,
        ethUsdtpool.address,
        chainlinkRegistryEthUsdt.address,
        await ethUsdtpool.token1(),
    ...
    it("should return correct token0 & token1 price in USD - pool of ETH-USDT - decimals 18-6 - twap for token1", async () => {
      ...
      if (poolToken0 == token1.address) {
        ...
        await chainlinkRegistryEthUsdt.setAnswer("300000000000", "0")
      } else {
        console.log("in else")
        ...
        await chainlinkRegistryEthUsdt.setAnswer("100000000", "0")
      }
    ...
    it("should return correct token0 & token1 price in USD - pool of ETH-WBTC - decimals 18-8 - twap for token0", async () => {
      ...
      // get uniswap pool instance
      let ethWbtcpool = (await ethers.getContractAt(
        "UniswapV3Pool",
        await uniswapV3Factory.getPool(token1.address, token2.address, "3000")
      )) as UniswapV3Pool

      let chainlinkRegistryWbtcUsdt = (await (await ChainlinkRegistryMockFactory).deploy(
        await ethWbtcpool.token0(),
        await ethWbtcpool.token1()
      )) as ChainlinkRegistryMock
      ...
      if (poolToken0 == token1.address) {
        console.log("in if")
        // initialize the pool
        await ethWbtcpool.initialize(encodePriceSqrt(expandTo18Decimals(12), "100000000"))
        ...
        await chainlinkRegistryWbtcUsdt.setAnswer("0", "3600000000000")
      } else {
        // initialize the pool
        await ethWbtcpool.initialize(encodePriceSqrt("100000000", expandTo18Decimals(12)))
        ...
        await chainlinkRegistryWbtcUsdt.setAnswer("0", "300000000000")
      }
      ...
      await periphery.swap(ethWbtcpool.address, false, "1000000", expandToString(sqrtPriceLimitX96))

      let useTwap: [boolean, boolean] = [true, false]
      const price0 = await oracleLibraryTest.getPriceInUSD(
        factory.address,
        ethWbtcpool.address,
        chainlinkRegistryWbtcUsdt.address,
        await ethWbtcpool.token0(),
        useTwap,
        strategyManager.address
      )
    ...
    it("should return correct token0 & token1 price in USD - pool of ETH-WBTC - decimals 18-8 - twap for token1", async () => {
      ...
      if (poolToken0 == token1.address) {
        ...
        await chainlinkRegistryWbtcUsdt.setAnswer("300000000000", "0")
      } else {
        // initialize the pool
        await ethWbtcpool.initialize(encodePriceSqrt("100000000", expandTo18Decimals(12)))
        ...
        await chainlinkRegistryWbtcUsdt.setAnswer("3600000000000", "0")
      }
  ...
  describe("#getPriceInUSD", async () => {
    it("should return correct price", async () => {
      ...
      // token 0 - chainink
      expect(
        await oracleLibraryTest.getPriceInUSD(
          factory.address,
          pool.address,
          chainlinkRegistry.address,
          await pool.token0(),
      ...
      ).to.equal("2999796379020818450736")
  ...
  describe("#allowSwap", async () => {
    it("should return false if swap is not allowed", async () => {
      expect(
        await oracleLibraryTest.allowSwap(
          pool.address,
          factory.address,
