...
const UniswapV3OracleTestFactory = ethers.getContractFactory("UniswapV3OracleTest")
...
const ChainlinkRegistryMockFactory = ethers.getContractFactory("ChainlinkRegistryMock")
...
import { calculateTick, encodePriceSqrt, expandTo18Decimals, expandToString } from "../utils"
...
describe("DefiEdgeTwapStrategy", () => {
  beforeEach(async () => {
    ...
    const uniswapV3Factory = (await (await UniswapV3FactoryFactory).deploy()) as UniswapV3Factory

    router = (await (await SwapRouterContract).deploy(uniswapV3Factory.address, weth9.address)) as SwapRouter
    ...
    // initialize the pool
    await pool.initialize(encodePriceSqrt(expandTo18Decimals(50000000), expandTo18Decimals(150000000000)))

    // deploy sharehelper library
    oracleLibrary = (await (await OracleLibraryLibrary).deploy()) as TwapOracleLibrary
    ...
    liquidityHelper = (await (await LiquidityHelperLibrary).deploy()) as LiquidityHelper
    ...
    strategyDeplopyer = (await (await DefiEdgeStrategyDeployerContract).deploy()) as DefiEdgeTwapStrategyDeployer

    chainlinkRegistry = (await (await ChainlinkRegistryMockFactory).deploy(
      await pool.token0(),
      await pool.token1()
    ...
    const DefiEdgeStrategyFactoryF = await ethers.getContractFactory("DefiEdgeTwapStrategyFactory")
    ...
    strategyManager = (await ethers.getContractAt("TwapStrategyManager", await strategy.manager())) as TwapStrategyManager
    ...
    oracle = (await (await UniswapV3OracleTestFactory).deploy()) as UniswapV3OracleTest
    ...
    await periphery.swap(pool.address, false, "10000000000000000000", expandToString(sqrtPriceLimitX96))
  ...
  describe("#Mint", async () => {
    ...
    it("should revert if caller have no access to strategy", async () => {
      ...
      await expect(strategy.connect(signers[3]).mint(expandTo18Decimals(1), expandTo18Decimals(3500), 0, 0, 0)).to.be.revertedWith("UA")
    })
    ...
    it("should not revert if strategy is public", async () => {
      ...
      await expect(strategy.connect(signers[3]).mint(expandTo18Decimals(1), expandTo18Decimals(3500), 0, 0, 0)).to.be.not.reverted
    })
    ...
    it("should revert if minted share is less than minimum share", async () => {
      ...
      await expect(strategy.mint(expandTo18Decimals(1), expandTo18Decimals(3500), 0, 0, expandTo18Decimals(5000))).to.be.revertedWith("SC")
    })

    it("should revert if minted amounts are less than minimum amounts", async () => {
      await expect(
        strategy.mint(expandTo18Decimals(1), expandTo18Decimals(3500), expandTo18Decimals(1), expandTo18Decimals(3500), 0)
    ...
    it("should revert if minted amounts exceeds maximum share mint limit", async () => {
      ...
      await expect(strategy.mint(expandTo18Decimals(1), expandTo18Decimals(3500), 0, 0, 0)).to.be.revertedWith("L")
    })

    it("should emit mint event with correct values", async () => {
      expect(await mint(signers[1]))
        .to.emit(strategy, "Mint")
        .withArgs(signers[1].address, "64672972976257143585", expandTo18Decimals(1), "3500000000000000000000")
    })

    it("revert if amount0 is 0 and amount1 is not 0", async () => {
      await approve(strategy.address, signers[0])

      await expect(strategy.mint(0, expandTo18Decimals(3500), 0, 0, 0)).to.be.revertedWith("INSUFFICIENT_AMOUNT")
    })

    it("if amount0 is not 0 and amount1 is 0 then transfer amount0 to strategy contract", async () => {
      await approve(strategy.address, signers[0])

      await expect(strategy.mint(expandTo18Decimals(1), 0, 0, 0, 0)).to.be.revertedWith("INSUFFICIENT_AMOUNT")
    })
  ...
  describe("#Burn", async () => {
    ...
    it("should not revert if strategy is public", async () => {
      ...
      await expect(strategy.connect(signers[0]).burn(shares, 0, 0)).to.be.not.reverted
    })

    it("should revert if msg.sender has no balance", async () => {
      expect(strategy.connect(signers[1]).burn(expandTo18Decimals(3000), 0, 0)).to.be.revertedWith("INS")
    })
    ...
    it("should calaculate unused balance while burning", async () => {
      ...
      await expect(strategy.connect(signers[0]).burn(shares, 0, 0))
        ...
        .withArgs(signers[0].address, "64672972976257143585", "994999984574006446", "3482499946777064187117")
    })
    ...
    it("should burn the liquidity with proper amounts when there is multiple ticks", async () => {
      ...
      await approve(strategy.address, signers[0])
      await strategy.connect(signers[0]).mint(expandTo18Decimals(1), expandTo18Decimals(3500), 0, 0, 0)
      ...
      await expect(strategy.connect(signers[0]).burn(shares, 0, 0))
        .to.emit(strategy, "Burn")
        .withArgs(signers[0].address, "129345946947514287170", "1989999984691828137", "6964999946421398503541")
    })
    ...
    it("should operator claim fee and burn token", async () => {
      ...
      await expect(claimFee).to.emit(strategy, "ClaimFee").withArgs("324989813951040922", "0")

      const sharesFeeto = (await strategy.balanceOf(signers[2].address)).toString()
      // const shares = "64199996762030683443";
    ...
    it("should revert if burned amounts are less than min amounts", async () => {
      ...
      expect(strategy.connect(signers[0]).burn(shares, "1099999999999999999", "3552260981108611397869")).to.be.revertedWith("S")
    })

    it("should decrease the total supply (burn shares)", async () => {
      const shares = (await strategy.balanceOf(signers[0].address)).toString()

      const totalSupplyBefore = parseInt((await strategy.balanceOf(signers[0].address)).toString())
      await strategy.connect(signers[0]).burn(shares, 0, 0)
      const totalSupplyAfter = parseInt((await strategy.balanceOf(signers[0].address)).toString())
    ...
    it("should emit burn event", async () => {
      ...
      await expect(strategy.connect(signers[0]).burn(shares, 0, 0))
        .to.emit(strategy, "Burn")
        .withArgs(signers[0].address, shares, "994999984691828143", "3482499946421398503516")
    })
    ...
    it("burn 25% - should calculate correct amount if there is unused balance", async () => {
      ...
      let returnAmount0 = new bn(amount0).multipliedBy(shareTobeBurned).dividedBy(shareTotalSupply).toFixed(0)
      let returnAmount1 = new bn(amount1).multipliedBy(shareTobeBurned).dividedBy(shareTotalSupply).toFixed(0)
      ...
      let totalAmount1 = new bn(unusedReturnAmount1).plus(returnAmount1).minus(3).toFixed(0)
      ...
      await expect(strategy.connect(signers[0]).burn(shareTobeBurned, 0, 0))
        .to.emit(strategy, "Burn")
        .withArgs(signers[0].address, shareTobeBurned, totalAmount0, totalAmount1)
    })
    ...
    it("burn different amount when performanceFeeRate is non-zero", async () => {
      ...
      await expect(strategy.connect(signers[0]).burn("60000000000000000000", 0, 0))
        .to.emit(strategy, "Burn")
        .withArgs(signers[0].address, "60000000000000000000", "923105840462252297", "3230870442330429811619")
    })
    ...
    it("burn different amount when performanceFeeRate is zero", async () => {
      ...
      await expect(strategy.connect(signers[0]).burn("60000000000000000000", 0, 0))
        .to.emit(strategy, "Burn")
        .withArgs(signers[0].address, "60000000000000000000", "923105840571560802", "3230870442000462902213")
    })
  })
  ...
  describe("#Rebalance - Hold", async () => {
    ...
    it("should burn all the liquidity", async () => {
      await strategy.rebalance("0x", [], [], true)
      const positionKey = getPositionKey(strategy.address, calculateTick(2500, 60), calculateTick(3500, 60))
  ...
  describe("#Redeploy", async () => {
    ...
    it("should mint liquidity to the ticks", async () => {
      const positionKey = getPositionKey(strategy.address, calculateTick(2500, 60), calculateTick(3300, 60))
      const position = await pool.positions(positionKey)
  ...
  describe("#emergencyWithdraw", async () => {
    it("should revert if caller is not governance", async () => {
      await expect(strategy.connect(signers[1]).emergencyWithdraw(token0.address, signers[0].address, expandTo18Decimals(1), [])).to.be
        .reverted
    })
    ...
    it("should revert if freeze emergency is activated", async () => {
      ...
      await expect(strategy.connect(signers[0]).emergencyWithdraw(token0.address, signers[0].address, 0, [])).to.be.reverted
    })

    it("should not revert if caller is governance & freeze emergency is not activated", async () => {
      await expect(strategy.emergencyWithdraw(token0.address, signers[0].address, 0, [])).to.be.not.reverted
    })
...
async function mint(signer: string | Signer | Provider) {
  ...
  return await strategy.connect(signer).mint(expandTo18Decimals(1), expandTo18Decimals(3500), 0, 0, 0)
}
