...
import { calculateTick, encodePriceSqrt, expandTo18Decimals, expandToString, encodePath } from "../utils"
...
describe("UniswapV3TwapLiquidityManager", () => {
  ...
  describe("#mintLiquidity", async () => {
    it("should emit mint event with correct values - strategy contract", async () => {
      expect(await mint(signers[0]))
        ...
        .withArgs(signers[0].address, "64672972976257143585", expandTo18Decimals(1), expandTo18Decimals(3500))
    })
  ...
  describe("#burnLiquidity", async () => {
    ...
    it("should emit burn event with correct values - strategy contract", async () => {
      ...
      expect(await strategy.connect(signers[0]).burn(shares, 0, 0))
        ...
        .withArgs(signers[0].address, shares, "994999984691828142", "3482499946421398503513")
    })

    it("should emit fees claimed event with correct values - strategy contract (burn)", async () => {
  ...
  describe("#burnAllLiquidity", async () => {
    ...
    it("should emit fees claimed event with correct values - strategy contract (rebalance)", async () => {
      await factory.changeFeeTo(signers[3].address)
  ...
  describe("#getAUMWithFees", async () => {
    ...
    it("should update liquidity amount", async () => {
      const positionKey = getPositionKey(strategy.address, calculateTick(2000, 60), calculateTick(4000, 60))
      ...
      expect(positionBefore.liquidity.toString()).to.equal("345732530090938345058")

      expect(positionAfter.liquidity.toString()).to.equal("345732530090938345058")
    })
  ...
  describe("#uniswapV3MintCallback", async () => {
    it("should revert if msg.sender is not uniswap v3 pool", async () => {
      expect(strategy.connect(signers[0]).uniswapV3MintCallback("0", "0", "0x")).to.be.reverted
    })
  })
